package geekbrains.lessons.homeworkfive;

public class Main {

    public static void main(String[] args) {
//        System.out.println(fact(10));
//        System.out.println(reFact(10));

//        System.out.println(fibo(7));
//        System.out.println(recFibo(45));

//        System.out.println(triangleNum(5));
//        System.out.println(recTriangleNum(5));
//
//        System.out.println(multiply(3,8));
//        System.out.println(recMultiply(3,8));

//        System.out.println(sumDigit(24523));
//        System.out.println(recSumDigit(24523));

        System.out.println(extentNum(8,4));
          System.out.println(recExtentNum(8,4));


    }

    public static int fact (int n){
        int value = 1;
        for (int i = 1; i <= n ; i++) {
            value *= i;
        }
        return value;
    }

    public static int reFact (int n){
        if (n <= 1){    //базовый случай
            return 1;
        }
        return reFact(n-1) * n; //рекурсивный случай
    }

    public static long fibo(int n){
        long a = 1;
        long b = 1;

        for (int i = 3; i <= n ; i++) {
            b = b + a;
            a = b - a;
        }
        return b;
    }

    public static long recFibo(int n){
        if(n < 3){     //базовый случай
            return 1;
        }
        return recFibo(n-1) + recFibo(n-2); //рекурсивный случай
    }

    public static int triangleNum(int n){
        int sum = 0;
        for (int i = 1; i <= n ; i++) {
            sum += i;
        }
        return sum;
    }

    public static int recTriangleNum(int n){
      if (n <= 1){
          return 1;
      }
      return recTriangleNum(n - 1) + n;
    }

    public static int multiply(int a, int b){
        int value = 0;
        for (int i = 0; i < b; i++) {
            value += a;
        }
        return value;
    }
    public static int recMultiply(int a, int b){
       if (b == 1){
           return a;
       }
       return recMultiply(a,b-1 + a);
    }

    public static int sumDigit(int n){
        int sum = 0;
        while (n > 0){
            sum += n % 10;
            n /= 10;
        }
        return sum;
    }

    public static int recSumDigit(int n){
        if (n <= 10){
            return n;
        }
        return recSumDigit(n/10) + n % 10;
    }

    public static int extentNum (int a, int n) {
        int result = 1;
        for (int i = 1; i <= n; i++) {
            result *= a;
        }
        return result;
    }

    public static int recExtentNum(int a, int n){
        if (n <= 1){
            return a;
        }
        return a * recExtentNum(a, n-1);
    }


}